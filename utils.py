...
def get_model(args, vocab_size, prompt_config=None):
    ...
    print_rank_0("set dropout for model to {:.3f}".format(config.dropout_rate))
...
def setup_model_and_optimizer(args, vocab_size, ds_config, prompt_config=None):
    ...
    print_rank_0("load from {}".format(args.load))
...
def save_ds_checkpoint(iteration, model, args, save_dir=None):
    ...
    model.save_checkpoint(args.save if save_dir is None else save_dir, str(iteration), client_state = sd)
